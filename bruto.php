<?php 
    ...
    if (isset($_GET['error']) && $_GET['error'] === 'neto_postoji') {
        echo "<p style='color: red;'>Ova bruto plaća se ne može obrisati. Postoji zapis o izračunatoj neto plaći za tog zaposlenika i mjesec. Molimo prvo obrišite zapis o neto plaći.</p>";
    }
...
<html>
    ...
    <script>
        ...
        var result = confirm("Jeste li sigurni da želite obrisati ovaj redak?");
            ...
            if(result){
                window.location.href = `obrisi_bruto.php?id=${zaposlenik}&mjesec=${mjesec}&godina=${godina}`;  
            }
        }
    ...
    <body>
        ...
        ?>
        <h1>Izračun bruto plaće po korisniku</h1>
        <br>
            <form method="get" action="<?php echo $_SERVER["PHP_SELF"];?>">
                ...
                <br>
                <br>
                <input name="submit" type="submit" value="Izračunaj bruto"/>
                ...
                <?php  
                    ...
                    if(isset($_GET["zaposlenik"]) && isset($_GET["mjesec"]) && isset($_GET["godina"])){
                        try{
                            ...
                            $upit = "INSERT INTO bruto_placa (zaposlenik, mjesec, godina)  VALUES (?, ?, ?)";
                            ...
                            echo "<br> Evidencija bruto plaće za " . $mjesec . " mjesec uspješno kreirana.";
                    ...
                    b.godina,
                    b.redovni_rad,
                    b.naknada_za_bolovanje,
                    b.naknada_za_godisnji_odmor,
                    b.naknada_za_placeni_dopust,
                    b.iznos
                    FROM bruto_placa b
                    LEFT JOIN zaposlenik z ON b.zaposlenik = z.oib
                    ORDER BY b.zaposlenik";
                    ...
                    if ($rezultat->rowCount() > 0) {
                        while ($red = $rezultat->fetch()) {
                            ...
                            echo "<td>{$red['redovni_rad']}</td>";
                            echo "<td>{$red['naknada_za_bolovanje']}</td>";
                            echo "<td>{$red['naknada_za_godisnji_odmor']}</td>";
                            echo "<td>{$red['naknada_za_placeni_dopust']}</td>";
